...
const convert = (
  ...
  const convertedColorScale = {};

  for (const color in colorScale) {
    const raw = colorScale[color];
    const converted = rgbConv(colorScale[color]);
    const hash = raw.slice(6, -1).split(" ").join(",").concat(",100");
    convertedColorScale[color] = [
      colorScale[color],
      formatRgb(converted),
      hash,
    ];
...
const formatScales = (
  colorMap: Map<string, Record<string, string[]>>,
): string => {
  const output: string[] = [];

  for (const [scale, colors] of colorMap) {
    output.push(`export const ${scale} = ${JSON.stringify(colors, null, 2)};`);
  }

  return output.join("\n\n");
};

const output = formatScales(colorMap).concat("\n");

console.log(output);

await Bun.write("./src/lib/vars/color-data.ts", output);
